    ...
    for i in range(num_iters):
        ...
        while (not done and num_steps < max_steps):
            ...
            if next_state != curr_state:
                ...
                q_hat[curr_state, action] = q_hat[curr_state, action] + alpha*(reward + gamma*np.max(q_hat[next_state,:]) - q_hat[curr_state, action])
...
def epsilon_greedy(q_hat, epsilon, state, action_space_size):
    ...
    # a certain threshold
    if (np.random.uniform(0,1) < epsilon or np.sum(q_hat[state,:]) == 0):
